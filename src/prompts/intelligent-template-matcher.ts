...
import { SimpleTemplate, PRACTICAL_TEMPLATES } from './reference-templates/template-library-simplified';
...
function analyzeLayoutComplexity(prd: PRDGenerationData): LayoutComplexityScore {
  ...
  const hasDataVisualization = requirements.some(req => 
    req.features.toLowerCase().includes('数据') ||
    req.features.toLowerCase().includes('统计') ||
    req.features.toLowerCase().includes('报表') ||
    req.features.toLowerCase().includes('图表')
  );
  
  // 检查是否有复杂导航需求
  const hasComplexNavigation = featureCount > 8 || scenarios.some(scenario =>
    scenario.scenario.includes('管理') ||
    scenario.scenario.includes('配置') ||
    scenario.scenario.includes('设置')
  );
...
export class IntelligentTemplateMatcher {
  ...
  private static exactMatch(prd: PRDGenerationData): TemplateMatchResult {
    const productDescription = [
      ...
      ...prd.requirementSolution.requirements.map(r => r.features),
      ...prd.userScenarios.map(s => s.scenario)
    ].join(' ').toLowerCase();
   ...
   * 5. 通用回退 - 最佳实践模板
   */
  private static genericFallback(): TemplateMatchResult {
    // 选择评分最高的通用模板
  ...
  private static getMatchTypeDescription(matchType: TemplateMatchResult['matchType']): string {
    const descriptions: Record<TemplateMatchResult['matchType'], string> = {
      'exact': '🎯 精确匹配',
      'functional': '🔧 功能相似性匹配',
      'layout': '📐 布局类型匹配',
      'hybrid': '🔀 混合策略匹配',
